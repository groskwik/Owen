...
int main() {
    double df,t,lambda,p,q,bound,gamm,bound1;
    double normq, n, k, mean, sd;
    int status,status1,choose;
    clear_screen();
    printf("STATISTICAL FACTORS FOR ONE-SIDED");
    printf("       TOLERANCE INTERVALS\n\n");
    printf("Number of observations:  n\n");
    printf("Proportion:              p\n");
    printf("Probability:         gamma\n\n");
    choose = 2 ;
    printf("Enter n: ");
    scanf("%lf", &n);
    printf("Enter p: ");
    ...
    sd = 1.0E0;
    sys_slowOff();
    cdfnor (&choose, &p, &q, &normq, &mean, &sd, &status1, &bound1);
    lambda = normq*sqrt(n);
    df = n-1;
    cdftnc (&choose, &gamm, &q, &t, &df, &lambda, &status, &bound);
    sys_slowOn();
    k = t / sqrt(n);
    printf("\nOwen k-factor: k = %.12f\n", k);
    sat_push_real(creal(n));
    sat_push_real(creal(p));
    sat_push_real(creal(gamm));
    sat_push_real(creal(k));
    WAIT_CANCEL;
    return 0;
}
